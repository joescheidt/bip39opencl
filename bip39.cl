...
bool normal_check(extended_private_key_t *input_key,__private uchar * target_address1,__private uchar * target_address2, uchar * res_address){
    extended_private_key_t target_key;
    for(int i=0; i<20; i++){
...
__kernel void int_to_address(__global uchar * res_address, __global uchar * target_mnemonic, __global uchar * found_mnemonic) {
  ...
  ushort indices[12] = {0};
  uchar target_address[20] = {0x27,0x60,0x98,0x93,0x8d,0x48,0xc5,0x5f,0x68,0x42,0x8e,0x48,0x88,0x46,0x27,0x01,0xcb,0x8b,0x52,0x62};
  ...
  bool found_target1 = 1;
  for(int j=0;j<20;j++) {
      if(hash160_address[j] != target_address[j]){
          found_target1 = 0;
      }
  }
  if(found_target1){
  ...
  uchar raw_address[20] = {0};
  hash160_eyas(&target_key_bip44, &raw_address);
